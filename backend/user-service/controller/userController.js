...
const login = async (req, res) => {
  ...
  if (getUser.exists) {
    ...
    if (isLoggedIn) {
      return res.status(403).send({ message: "This user is already logged in."
        + "\nPlease terminate the current login session you might have on another device or browser." })
    }
    ...
    bcrypt.compare(req.body.password, userPassword, (error, compareResult) => {
      ...
      jwt.sign(payload, JWT_SECRET, { algorithm: "HS256" }, (error, token) => {
        ...
        // update login status of this user
