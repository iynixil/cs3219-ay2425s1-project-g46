...
const login = async (req, res) => {
  ...
  const getUser = await usersRef.get();
  if (getUser.exists) {
    // check if user currently has another login session
    const isLoggedIn = getUser.get("isLoggedIn");

    if (isLoggedIn) {
      return res.status(403).send({
        message: "This user is already logged in." +
          "\nPlease terminate your current session that might be in another browser or device."
      })
    }
    ...
    bcrypt.compare(req.body.password, userPassword, (error, compareResult) => {
      ...
      jwt.sign(payload, JWT_SECRET, { algorithm: "HS256" }, (error, token) => {
        ...
        // set the login status of this user to true
        usersRef.update({ "isLoggedIn": true });
        // log message of the user logging in
...
const logout = async (req, res) => {
  // set the login status of this user to false
  const usersRef = userCollection.doc(req.body.email);
  usersRef.update({ "isLoggedIn": false });

  // log message of the user logging out
  console.log("User " + req.body.email + " is logging out.");
